...
Broadly scans the official SAM.gov Contract Opportunities Public API, scores
recent opportunities for "low-hanging-fruit" supply/equipment potential, and
emails a PDF digest plus a CSV audit trail.
...
INCLUDE_SOURCES_SOUGHT           default: true
...
STATE_FILE = SCRIPT_DIR / "sam_seen_opportunities_v2.json"
...
INCLUDE_SOURCES_SOUGHT = env_bool("INCLUDE_SOURCES_SOUGHT", True)
...
NOTICE_TYPES: tuple[str, ...] = (
    ("p", "k", "o", "r") if INCLUDE_SOURCES_SOUGHT else ("p", "k", "o")
)
...
class Evaluation:
    ...
    def is_actionable(self) -> bool:
        return self.bucket in {"strong", "conditional", "pipeline"}
...
def evaluate_opportunity(
    ...
    if notice_type_code in {"o", "k"}:
        evaluation.score += 1
        evaluation.reasons.append("Open solicitation stage.")
    elif notice_type_code == "r":
        evaluation.score -= 1
        evaluation.warnings.append(
            "Sources Sought is an early-pipeline item, not a quote request."
        )
    ...
    elif notice_type_code in {"p", "r"}:
        if evaluation.score >= MIN_SCORE_CONDITIONAL:
            evaluation.bucket = "pipeline"
        else:
            evaluation.bucket = "rejected"
            evaluation.rejection_reasons.append(
                f"Pipeline score {evaluation.score} is below "
...
def evaluate_all(
    ...
    evaluations.sort(
        key=lambda item: (
            {"strong": 0, "conditional": 1, "pipeline": 2, "rejected": 3}[
                item.bucket
            ],
            -item.score,
            item.sort_deadline,
...
def write_pdf(
    ...
    story.append(Paragraph("SAM.gov Low-Hanging-Fruit Opportunities", styles["Title"]))
    story.append(
        Paragraph(
            paragraph_safe(
                ...
                f"Pipeline: {bucket_counts['pipeline']} | "
    ...
    if not included:
        ...
        for bucket, heading in (
            ("strong", "Strong Pursuits"),
            ("conditional", "Conditional Pursuits"),
...
def build_email_body(
    ...
    return (
        ...
        f"- Early pipeline: {counts['pipeline']}\n\n"
        f"All fetched notices evaluated:\n"
        f"- Strong: {fetched_counts['strong']}\n"
        f"- Conditional: {fetched_counts['conditional']}\n"
        f"- Early pipeline: {fetched_counts['pipeline']}\n"
